...
DCP.UOS_collect = function UOS_collect(start_num,stop_num){
    ...
    var pre_DB = JSON.parse(DCP.getDB("UOS_DB")) // DB 읽기 작업
    for(let i=start_num;i<stop_num+1;i++){
        try{
            ...
            if(i%50==0){
                ...
                DCP.setDB("UOS_DB",JSON.stringify(pre_DB)) // DB 쓰기 작업
                pre_DB = JSON.parse(DCP.getDB("UOS_DB")) // DB 읽기 작업
                java.lang.Thread.sleep(30000) // 200ms
            }
            count_sucess++
            java.lang.Thread.sleep(2000) // 200ms
...
DCP.UOS_search = function UOS_search(key,mod){
    ...
    if(mod=="title"){
        return JSON.parse(DCP.getDB("UOS_DB")).filter(v=>String(v[0]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0})
            .map(v=>"제목 : "+v[0]+"\n작성자 : "+v[3]+" "+v[4]+"\n날짜 : "+v[2]+"\n- https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("").trim().substr(0,200000)

    }
    else if(mod=="writer"){
        return JSON.parse(DCP.getDB("UOS_DB")).filter(v=>String(v[3]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0})
            .map(v=>"제목 : "+v[0]+"\n작성자 : "+v[3]+" "+v[4]+"\n날짜 : "+v[2]+"\n- https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("").trim().substr(0,200000)
