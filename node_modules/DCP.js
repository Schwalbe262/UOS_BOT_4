...
// 검색코드 예시 : JSON.parse(DCP.getDB("DCP_DB")).filter(v=>String(v[0]).replace(/ /g,"").includes("시립")).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0}).map(v=>v[0]+" ("+v[2]+")\n- "+"https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("").trim()
DCP.DCP_collect = function DCP_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        try{
            ...
            try{ var pre_DB = JSON.parse(DCP.getDB("DCP_DB"))} catch(e){}
            pre_DB.push(array)
            DCP.setDB("DCP_DB",JSON.stringify(pre_DB))
            ...
            count_sucess++
            java.lang.Thread.sleep(100) // 200ms
