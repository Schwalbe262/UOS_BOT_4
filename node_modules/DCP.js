...
DCP.UOS_collect = function UOS_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        try{
            ...
            try{ var ip = body.select(".gallview_head>.gall_writer>.fl>.ip").get(0).ownText() }catch{ var ip = "" }
...
DCP.UOS_search = function UOS_search(key){
    return JSON.parse(DCP.getDB("UOS_DB")).filter(v=>String(v[0]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0}).map(v=>"제목 : "+v[0]+" \n작성자 : "+v[3]+" ("+v[2]+")\n- "+"https://gall.dcinside.com/board/view/?id=uos&no="+v[1]+"\n\n").join("").trim()
}
