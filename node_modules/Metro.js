...
Metro.parse = function parse(station){
    ...
    array.push( pre_array.filter(v=>String(v[4])==String(station_name+" 도착")).sort(function(a, b) { // 오름차순
        return Number(a[1]) < Number(b[1]) ? -1 : Number(a[1]) > Number(b[1]) ? 1 : 0;
        // 광희, 명수, 재석, 형돈
    }) )

    array.push( pre_array.filter(v=>String(v[4])==String(station_name+" 진입")) )

    array.push( pre_array.filter(v=>String(v[4])==String(station_name+" 전역 출발")) )

    array.push( pre_array.filter(v=>String(v[4])==String(station_name+" 전역 도착")) )

    array.push( pre_array.filter(v=>String(v[4])==String(station_name+" 전역 진입")) )
