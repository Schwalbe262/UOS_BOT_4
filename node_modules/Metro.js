...
Metro.parse = function parse(station){
    ...
    array = array.concat( pre_array.filter(v=>String(v[4])==String(station_name+" 도착")) )

    array = array.concat( pre_array.filter(v=>String(v[4])==String(station_name+" 진입")) )

    array = array.concat( pre_array.filter(v=>String(v[4])==String("전역 출발")) )

    array = array.concat( pre_array.filter(v=>String(v[4])==String("전역 도착")) )

    array = array.concat( pre_array.filter(v=>String(v[4])==String("전역 진입")) )

    for (let i=0 ; i<totalCount ; i++){
        try{
            let temp = Number( String(pre_array[i][4]).split("]번째")[0].split("[")[1] ) * 180
            if(isNaN(temp)==false){ pre_array[i][4] == temp }
        }catch(e){}
    ...
    for (let i=0 ; i<totalCount ; i++){
        try{
            let temp = Number( String(pre_array[i][4]).split("분")[0] ) * 60
                + Number( String(pre_array[i][4]).split("초")[0].split("분 ")[1] )
            if(isNaN(temp)==false){ pre_array[i][4] == temp }
        }catch(e){}
    }

    pre_array = pre_array.filter(v=>isNaN(v[4])==false)

    array = array.concat( pre_array.sort(function(a,b){return Number(a[1])<Number(b[1]) ? -1 : Number(a[1]) > Number(b[1]) ? 1 : 0}) )
