...
Metro.test = function test(station){
    ...
    /*
     "rowNum": "1",
                ...
                "statnNm": "청량리",
                "ordkey": "01001양주0",
                "subwayList": "1001,1063,1067,1075", // 그 역에 들어오는 노선 정보
    ...
    let train_num = temp.map(v=>v.brainNo) // 열차번호
    let last_station = temp.map(v=>v.bstatnNm) // 종착역

    let stationFid = temp.map(v=>v.stationFid) // 현재위치
    let stationTid = temp.map(v=>v.stationTid) // 열차번호
    let stationId = temp.map(v=>v.stationId) // 종착역

    let array = []

    for (let i=0 ; i<totalCount ; i++){
        array[i] = [number[i],line_number[i],updn[i],arrival_msg2[i],arrival_msg3[i],last_station[i],stationFid[i],stationTid[i],stationId[i]].join("||")
    }
