...
Metro.parse = function parse(station){
    ...
    for (let i=0 ; i<totalCount ; i++){
        try{
            let temp = 0
                try{ temp = temp + Number( String(unique_array[i][4]).split("분")[0] ) * 60} catch(e){}
                try{ if(unique_array[i][4].indexOf("초")!=-1){temp = temp + Number( String(unique_array[i][4]).split("초")[0].split("분 ")[1] )}}catch(e){}
            if(isNaN(temp)==false){ unique_array[i][4] = temp }
        }catch(e){}
    }

    try{
        unique_array = unique_array.filter(v=>isNaN(v[4])==false) //  도착 예정 시간이 숫자가 아닌 것들은 제거 (도착 진입 출발 등등의 것)
