...
Metro.output = function output(room,station){
    ...
    let station_name
    if(station.substr(station.length-1,1) == "역" && station != "서울역" ){
    ...
    let header_title = station_name+" 검색 결과"
    let header_mobile_link = "redirect_UOS?tourl=https://www.sisul.or.kr/open_content/skydome/images/img_subway.png"
    let header_web_link = "redirect_UOS?tourl=https://www.sisul.or.kr/open_content/skydome/images/img_subway.png"
    ...
    for(let i=0 ; i<5 ; i++){
        try{
            ...
            sub_title[i] = String(data[i][10]).replace("[","").replace("]","") // 1호선 하행 (5분 4초후)
    ...
    let NEC = ["redirect_UOS?tourl=http://15.164.226.64:7080/image/animal"+String(Math.floor(Math.random() *10 ))+".jpg","redirect_UOS?tourl=http://15.164.226.64:7080/image/animal"+String(Math.floor(Math.random() *10 ))+".jpg","1"] // 동물사진
...
String.prototype.reduceMetro = function(){ // 역명 Api 호환 형태로 변경
    ...
    else if(this=="응암순환"||this=="상선"){return "응암순환(상선)"}
...
String.prototype.lineLink = function(){
    ...
    else if(this=="1067"){return "redirect_UOS?tourl=http://15.164.226.64:7080/image/subway_map_lineKK.png"}
...
Metro.sendKaling_universal_5list = function sendKaling_universal_5list(room,header,title,sub_title,link,mobile_link,THU,NEC){
    ...
    let kalingObj = {
        ...
        "template_args" : {
            ...
            listRank : String(NEC[2]) // true false
        }
    }
    try{
