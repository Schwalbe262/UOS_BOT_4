...
Metro.parse = function parse(station){
    ...
    let unique_array = pre_array.reduce(function(a,b){
        if (a[3].indexOf(b[3]) < 0) a.push(b)
        return a
    },[])
    ...
    for (let i=0 ; i<totalCount ; i++){
        try{
            let temp = Number( String(unique_array[i][4]).split("]번째")[0].split("[")[1] ) * 180
    ...
    array = array.concat( unique_array.sort(function(a,b){return Number(a[1])<Number(b[1]) ? -1 : Number(a[1]) > Number(b[1]) ? 1 : 0}) )
