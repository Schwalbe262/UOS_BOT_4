...
UOSP.UOSP1_collect = function UOSP1_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num;i++){
        try{
            let body = org.jsoup.Jsoup.connect("http://www.uos.ac.kr/korNotice/view.do?list_id=FA1&seq=7254").get()
            ...
            let pre_DB = JSON.parse(getDB("UOP1_DB"))
            pre_DB.push(array)
            setDB("UOSP1_DB",JSON,stringify(pre_DB))
            if(i%100==0){Api.replyRoom(console_room_name,i+"번 주소까지 진행 완료")}
        }
