...
UOSP.UOS_temp_controller_biz = function UOS_temp_controller_biz(){
    ...
    if(result!=undefined){
        ...
        let URL = String("korNotice/list.do?list_id=20008N2&epTicket=INV")
    ...
    if(length==5||mod=="ON"){ // 데이터가 5개가 쌓이면 리턴을 하지 않고 그냥 바로 공지로 내보냄
        ...
        for(let i=0;i<5;i++){
            if(return_data[i]==undefined){
                ...
                URL[i] = String("social/korNotice/allList.do?list_id=econo01&cate_id2=000010057&epTicket=ST-331566-BHszXHC0b5FdFp1RjlWhASutG0tvCmiNkIE-22")
                sub_title[i] = String("경제학부")
            }
            else if(return_data[i][0]=="4"){
                URL[i] = String("social/korNotice/allList.do?list_id=econo01&seq=1946&sort=0&pageIndex=1&searchCnd=&searchWrd=&viewAuth=Y&cate_id2=000010072&epTicket=ST-331596-moOaQeUAwN1abnAplueCx7KOeWTjskUrALK-22")
                sub_title[i] = String("사회복지학과")
            }
            else if(return_data[i][0]=="5"){
                URL[i] = String("social/korNotice/allList.do?list_id=econo01&seq=1958&sort=0&pageIndex=1&searchCnd=&searchWrd=&viewAuth=Y&cate_id2=000010074&epTicket=ST-331603-QDYHuHqTOlcLyAXnnIYRKWTORGh77Cw2kj6-22")
