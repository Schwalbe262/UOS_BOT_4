...
UOSP.UOSP1 = function UOSP1(){ // 일반공지 파싱
    Api.replyRoom(console_room_name,"UOSP1함수진입")
    let str = ""
    ...
    if (SW_new == 1) {
        ...
        for(let i=0;i<Number(roomListNum);i++){
            try{UOSP.sendKaling_notice(roomList[i],"일반공지알림","","",title)}
            catch(e){}
        }
    }
}
...
UOSP.UOSP1_collect = function UOSP1_collect(start_num,stop_num){
    Api.replyRoom(console_room_name,"collect함수진입")

    let count_sucess = 0
    let count_failed = 0

    // 초기 DB 세팅 코드 : UOSP.setDB("UOSP1_DB",JSON.stringify([])) // 주의 : 건드리지 말 것
    Api.replyRoom(console_room_name,"getDB")
    var pre_DB = JSON.parse(UOSP.getDB("UOSP1_DB"))
    Api.replyRoom(console_room_name,"getDB완료")
    for(let i=Number(start_num);i<Number(stop_num)+1;i++){
        try{
            Api.replyRoom(console_room_name,"try")
            ...
            Api.replyRoom(console_room_name,"array 세팅")
            let array = [[String(title).replaceAmp()],[i],[date]]
            Api.replyRoom(console_room_name,"array 세팅완료")
    ...
    Api.replyRoom(console_room_name,"stringify 수행")
    let temp = JSON.stringify(pre_DB)
    Api.replyRoom(console_room_name,"setDB")
    UOSP.setDB("UOSP1_DB",temp)
    Api.replyRoom(console_room_name,"setDB완료")
...
UOSP.sendKaling_notice = function sendKaling_notice(room, title, description, THU, head){
    let kalingObj = {
        "link_ver":"4.0",
        "template_id" : (33040),
        "template_args" : {
...
UOSP.setDB = function setDB(key,value){
    Api.replyRoom(console_room_name,"setDB진입")
    if(D.selectForArray("UOSPTable", null, "k like ?", key).length == 0){ // 해당하는 key에 value가 없는 경우
        ...
        Api.replyRoom(console_room_name,"DB update")
        D.update("UOSPTable", {v:value}, "k=?", [key])
        Api.replyRoom(console_room_name,"DB update 완료")
    }
}
