...
UOSP.UOSP1_collect = function UOSP1_collect(start_num,stop_num){
    ...
    for(let i=start_num;i<stop_num+1;i++){
        try{
            ...
            try{ var pre_DB = JSON.parse(UOSP.getDB("UOSP1_DB"))} catch(e){}
            pre_DB.push(array)
...
UOSP.UOSP1_search = function UOSP1_search(key){
    return JSON.parse(UOSP.getDB("UOSP1_DB")).filter(v=>String(v[0]).replace(/ /g,"").includes(key.replace(/ /g,""))).sort(function(a,b){return Number(a[1])>Number(b[1]) ? -1 : Number(a[1]) < Number(b[1]) ? 1 : 0}).map(v=>v[0]+" ("+v[2]+")\n- "+"http://www.uos.ac.kr/korNotice/view.do?list_id=FA1&seq="+v[1]+"\n\n").join("").trim()
}
