...
UOSP.UOS_KY = function UOS_KY(){ // 경영학부 공지 파싱 (어레이 반환)
    ...
    let KY_DB = JSON.parse(UOSP.getDB("UOS_KY_DB"))

    // DB구조 : [공지제목,공지제목,공지제목]
    let pre_DB = KY_DB[KY_DB.length-1] // 이전 DB 마지막 부분
    ...
    if (SW_new == 1) {
        return_array = [[String(opt)],[String(title)]]
        KY_DB.push(return_array)
        UOSP.setDB("UOS_KY_DB", JSON.stringify(KY_DB))
        return return_array
    }
    return undefined
}
...
UOSP.UOS_temp_controller = function UOS_temp_controller(opt,mod){
    ...
    if(Number(opt)==7){result = UOSP.UOS_KY()}
    ...
    if(length==5||mod=="ON"){ // 데이터가 5개가 쌓이면 리턴을 하지 않고 그냥 바로 공지로 내보냄
        ...
        for(let i=0;i<5;i++){
            if(return_data[i]==undefined){
                ...
                URL[i] = String("https://biz.uos.ac.kr/korNotice/list.do?list_id=20008N2&epTicket=INV")
        ...
        if(mod!="ON"){
            for(let i=0;i<Number(roomListNum);i++){
                try{UOSP.sendKaling_5list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                catch(e){
            ...
            for(let i=0;i<Number(roomListNum);i++){
                try{
                    if(length==1){UOSP.sendKaling_1list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                    else if(length==2){UOSP.sendKaling_2list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                    else if(length==3){UOSP.sendKaling_3list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                    else if(length==4){UOSP.sendKaling_4list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                    else if(length==5){UOSP.sendKaling_5list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                }
                catch(e){
                    Api.replyRoom(console_room_name, "Kaling Error\n" + e + "\n" + e.stack + "\n" + e.rhinoException);
                    Api.replyRoom(console_room_name,"roomList : "+roomList[i])
...
UOSP.sendKaling_4list = function sendKaling_5list(room, header_title, title, sub_title, path, THU){
...
UOSP.sendKaling_3list = function sendKaling_5list(room, header_title, title, sub_title, path, THU){
...
UOSP.sendKaling_2list = function sendKaling_5list(room, header_title, title, sub_title, path, THU){
...
UOSP.sendKaling_1list = function sendKaling_5list(room, header_title, title, sub_title, path, THU){
