...
UOSP.UOS_temp_controller_biz = function UOS_temp_controller_biz(opt,mod){

    let result = UOSP.UOS_biz(opt)

    let return_data
    if(result!=undefined || mod=="ON"){
        let pre_DB = JSON.parse(UOSP.getDB("UOSP_temp_biz"))
        if(result!=undefined){pre_DB.push(result)}
        return_data = pre_DB
        UOSP.setDB("UOSP_temp_biz",JSON.stringify(return_data))
    }
    ...
    if(length==1||mod=="ON"){ // 데이터가 1개가 쌓이면 리턴을 하지 않고 그냥 바로 공지로 내보냄
        ...
        for(let i=0;i<1;i++){
            if(return_data[i]==undefined){
                ...
                URL[i] = String("korNotice/list.do")
                sub_title[i] = String("경영학부")
            }
        }
        let THU = ""
        let header_title = "경영학부공지"

        if(mod=="ON"){
            for(let i=0;i<Number(roomListNum);i++){
                try{UOSP.sendKaling_1list_biz(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                catch(e){}
            }
        }
