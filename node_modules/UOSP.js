...
var roomList = ["시립봇4 컨트롤방","노은총","시립대공지확인방"]
...
UOSP.UOSP3 = function UOSP3(){
    ...
    if ( pre_DB[1] < adress ) { // 새로운 내용이 파싱되었을 경우
        ...
        SW_new = 1
        if(title.indexOf("인턴")!=-1 || title.indexOf("근로")!=-1){
            SW_2 = 1
        }
    }
...
UOSP.UOSP5 = function UOSP5(){
    ...
    if (SW_new == 1) {
        for(let i=0;i<Number(roomListNum);i++){
            try{UOSP.sendKaling_notice(String(roomList[i]),"장학공지알림",String("redirect_UOS?tourl=https://scholarship.uos.ac.kr/scholarship/notice/notice/list.do?brdBbsseq=1"),"","",String(title))}
            catch(e){}
        }
    }
}
