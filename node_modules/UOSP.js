...
UOSP.UOS_engineering = function UOS_engineering(opt){ // 공과대학 최신공지 제목을 파싱하여 어레이로 반환
    ...
    if ( pre_DB != String(title).replaceAmp()) { SW_new = 1 } // 새로운 내용이 파싱되었을 경우 SW 1

    if (SW_new == 1) {
        return_array = [String(opt),String(title)]
        UOSP.setDB("UOS_engineering"+opt+"_DB", JSON.stringify(engineering_DB.push(title)))
        return return_array
    }
    return undefined
}
...
UOSP.UOS_temp_controller = function UOS_temp_controller(opt){
    ...
    if(result!=undefined){
        ...
        return_data = pre_DB.push(result)
    ...
    if(return_data.length=5){ // 데이터가 5개가 쌓이면 리턴을 하지 않고 그냥 바로 공지로 내보냄
        ...
        for(let i=0;i<Number(roomListNum);i++){
            try{UOSP.sendKaling_5list(String(roomListNum[i]), header_title, title, sub_title, path, THU)}
            catch(e){}
        }
