    ...
    if(return_data.length==5){ // 데이터가 5개가 쌓이면 리턴을 하지 않고 그냥 바로 공지로 내보냄
        ...
        for(let i=0;i<5;i++){
            if(return_data[i][0]=="1"){
                URL[i] = "engineering/korNotice/allList.do?list_id=20013DA1&"
                sub_title[i] = "전자전기컴퓨터공학부"
            }
            else if(return_data[i][0]=="2"){
                URL[i] = "engineering/korNotice/allList.do?list_id=20013DA1&cate_id"
                sub_title[i] = "화학공학과"
            }
            else if(return_data[i][0]=="3"){
                URL[i] = "engineering/korNotice/allList.do?list_id=20013DA1&cate_id2=000010059&epTicket=INV"
                sub_title[i] = "기계정보공학과"
            }
            else if(return_data[i][0]=="4"){
                URL[i] = "engineering/korNotice/allList.do?list_id=20013DA1&cate_id2=000010061&epTicket=INV"
                sub_title[i] = "신소재공학과"
            }
            else if(return_data[i][0]=="5"){
                URL[i] = "engineering/korNotice/allList.do?list_id=20013DA1&cate_id2=000010382&epTicket=INV"
                sub_title[i] = "토목공학과"
            }
            else if(return_data[i][0]=="6"){
                URL[i] = "engineering/korNotice/allList.do?list_id=20013DA1&cate_id2=000010383&epTicket=INV"
                sub_title[i] = "컴퓨터과학부"
            }
        }
        let THU = ""
        ...
        for(let i=0;i<Number(roomListNum);i++){
            try{UOSP.sendKaling_5list(String(roomList[i]), header_title, title, sub_title, URL, THU)}
                ...
                Api.replyRoom(console_room_name,"카링에러")
