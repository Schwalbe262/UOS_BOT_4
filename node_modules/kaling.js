...
exports.Kakao = function() {
    ...
    Kakao.prototype.init = function(apiKey) {
        ...
        //this.apiKey = apiKey;
        this.apiKey = "904c559bc0ec53c94df6dad990b24c52";
        return this;
    }
    ...
    Kakao.prototype.static = {
        ...
        ka: 'sdk/1.36.6 os/javascript lang/en-US device/Win32 origin/http%3A%2F%2Fportal.uos.ac.kr'
    }
    Kakao.prototype.login = function(id, password) {
        ...
        (function authenticate() {
            ...
            connection.data({
                os: 'web',
                webview_v: '2',
                email: CryptoJS.AES.encrypt(id, String(this.cryptoKey)).toString(),
                password: CryptoJS.AES.encrypt(password, String(this.cryptoKey)).toString(),
    ...
    Kakao.prototype.send = function(roomTitle, data, type) {
        ...
        (function getRooms() {
            ...
            connection.ignoreContentType(true);
            const response = connection.execute();
            const document = String(response.body()).replace(/ /g,'');
            this.rooms = JSON.parse(document);
        }).bind(this)();

        (function sendTemplate() {
            var id, securityKey;
            for(var roomIdx in this.rooms.chats) {
                var room=this.rooms.chats[roomIdx];
                if(room.title === roomTitle.replace(/ /g,"")) {
                    id = room.id;
                    securityKey = this.rooms.securityKey;
                    break;
                }
            }
            if(typeof id === 'undefined') throw new ReferenceError('방제목이 올바르지 않습니다. ' + roomTitle.replace(/ /g,""));
