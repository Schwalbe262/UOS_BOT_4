...
UOS_time.UOS_Time_DB_search_toString_detail = function UOS_Time_DB_search_toString_detail(Num,room){
    ...
    if(Num%1==0){
        try{
            var search_nm = UOS_time.getDB("search_detail_"+room).split("||")[Num-1]
...
UOS_time.setDB = function setDB(key,value){
    if(D.selectForArray("UOS_Time_kv", null, "k like ?", key).length == 0){ // 해당하는 key에 value가 없는 경우
        D.insert("UOS_Time_kv",{k:key, v:value})
    }
    else{ // 해당하는 key에 value가 있는 경우
        D.update("UOS_Time_kv", {v:value}, "k=?", [key])
    }
}


UOS_time.getDB = function getDB(key){
    var arr = D.selectForArray("UOS_Time_kv", null, "k=?", [key]);
    if (arr.length > 0) {
        return arr[0][1];
    } else {
        return undefined;
